...
Advisor Prep Agent transforms the tedious process of meeting preparation into a streamlined, AI-assisted workflow. Instead of manually parsing dozens of unstructured files, advisors can use the system to automatically extract key financial context and generate a prioritized meeting agenda.

### Key Features

*   **Explainable RAG (Retrieval-Augmented Generation)**: Every AI-generated insight is strictly tied to a specific source document and page number. Advisors can instantly verify any point using the integrated split-screen document viewer.
*   **Human-in-the-Loop**: The system acts as a data synthesizer, not a decision-maker. Advisors must explicitly approve or discard each AI-generated insight, ensuring human judgment and empathy remain central to the client relationship.
*   **Local-First & Data-Private**: Designed to run locally with a ChromaDB vector store. The architecture is built to support local LLMs to maximize data privacy in sensitive financial contexts.
*   **Actionable Outputs**: Consolidates information from emails, tax documents, and spreadsheets into a clean, exportable agenda ready for CRM entry or client meetings.
